...
- DB 스키마, 마이그레이션, 저장소 구현
- 브로커 포트나 KIS 연동
- 시장 데이터, 후보 선정, 필터, 의사결정 및 주문 의도
- 주문, 체결, 포지션, 손익, 자산 곡선 및 이벤트 테이블
- 텔레그램, 알림 outbox, 스케줄러, 분석 CLI, CSV, UI
- 실매매와 모든 외부 네트워크 호출

## 다음 단계와 제한사항

다음 PR에서는 현재 도메인 타입을 기반으로 애플리케이션 유스케이스와 영속성 경계를
설계합니다. PR 1에는 거래 세션 달력, DST/휴장일 처리, 통화별 소수 자릿수, UUIDv7,
